...
await writeJson(path.join(statusRoot, "state/benchmarks/last-benchmark-cycle.json"), {
  ...
  multiCycle: {
    ...
    windowHistorySummary: {
      ...
      dominantDirection: "improving",
      averageDelta: 2.63,
      volatility: 2.63,
      summary: "Recent window comparisons skew improving (2 improving vs 0 degrading) with an average delta of 2.63."
    },
    confidence: {
      score: 90,
      level: "high",
...
assert.ok(runtimeStatus.benchmarkSummary.topWasteDomains[0].evidenceScore > 0);
assert.ok(["low", "medium", "high"].includes(runtimeStatus.benchmarkSummary.topWasteDomains[0].evidenceBand));
assert.match(runtimeStatus.benchmarkSummary.topWasteDomains[0].expectedSavingsHint, /recover roughly/i);
assert.match(runtimeStatus.benchmarkSummary.topWasteDomains[0].expectedOutcome, /cost hotspot|reduce live spend/i);
assert.match(runtimeStatus.benchmarkSummary.topWasteDomains[0].impactSummary, /live tokens|threshold/i);
...
assert.ok(runtimeStatus.benchmarkSummary.safeSavingsOpportunities[0].evidenceScore > 0);
assert.ok(["low", "medium", "high"].includes(runtimeStatus.benchmarkSummary.safeSavingsOpportunities[0].evidenceBand));
assert.match(runtimeStatus.benchmarkSummary.safeSavingsOpportunities[0].expectedSavingsHint, /recover roughly/i);
assert.match(runtimeStatus.benchmarkSummary.safeSavingsOpportunities[0].expectedOutcome, /low-risk|auto-tune/i);
assert.match(runtimeStatus.benchmarkSummary.safeSavingsOpportunities[0].impactSummary, /live tokens|threshold/i);
...
assert.equal(runtimeStatus.benchmarkCycleSummary.windowHistorySummary.available, true);
assert.equal(runtimeStatus.benchmarkCycleSummary.windowHistorySummary.dominantDirection, "improving");
assert.match(runtimeStatus.benchmarkCycleSummary.windowHistorySummary.summary, /improving|window comparisons/i);
...
assert.ok(runtimeStatus.nextActions[0].evidenceScore > 0);
assert.ok(["low", "medium", "high"].includes(runtimeStatus.nextActions[0].evidenceBand));
...
assert.ok(typeof runtimeStatus.nextActions[0].impactSummary === "string");
...
assert.ok(runtimeDoctor.recommendedActions[0].evidenceScore > 0);
...
assert.ok(runtimeReport.economics.topWasteDomains[0].evidenceScore > 0);
assert.ok(["low", "medium", "high"].includes(runtimeReport.economics.topWasteDomains[0].evidenceBand));
assert.ok(typeof runtimeReport.economics.topWasteDomains[0].expectedOutcome === "string");
assert.ok(typeof runtimeReport.economics.topWasteDomains[0].impactSummary === "string");
...
assert.ok(runtimeReport.controls.nextActions[0].evidenceScore > 0);
assert.ok(["low", "medium", "high"].includes(runtimeReport.controls.nextActions[0].evidenceBand));
assert.ok(typeof runtimeReport.controls.nextActions[0].impactSummary === "string");
...
assert.equal(runtimeReport.evidence.longRun.windowHistorySummary.available, true);
assert.equal(runtimeReport.evidence.longRun.windowHistorySummary.dominantDirection, "improving");
...
assert.equal(benchmarkCycle.multiCycle.windowHistorySummary.available, true);
assert.ok(["improving", "degrading", "flat"].includes(benchmarkCycle.multiCycle.windowHistorySummary.dominantDirection));
