...
- `multiCycle` now also keeps `windowHistorySummary`, so status/report views can explain whether recent window comparisons are mostly improving, degrading, or still too mixed to trust
...
- workflow preview phases now also carry `objective`, `readyState`, `gatingReason`, and `expectedDeltaCard`, so the operator can see whether a phase is ready and what bounded delta it is expected to move before applying it
- phase cards in `report` now also carry a small `deltaCard`, so the operator can see estimated token upside, affected domains, and how many changes are applyable before touching the phase
